...
def get_question_answer_pairs(question_file, is_test=False,qaf_file =''):
    qapairs = []
    ...
    for line in fqa:
        ...
        cols = line.split(b",")
        q=cols[9].split(b'\t')
        question = q[0].strip().decode('utf8')
        qwords = nltk.word_tokenize(question)
        if not is_test:
            correct_ans = cols[3].decode('utf8').strip().upper()
            ##print(correct_ans,"\n\n")
            answers = q[1:]
            # training file parsing
            correct_ans_idx = ord(correct_ans) - ord('A')
            for idx, answer in enumerate(answers):
                answer=re.sub('\(\w\)','',answer.decode('utf8')).strip()
...
def build_vocab(stories, qapairs, testqs):
    ...
    word2idx = {w: ((i+1)/(len(words))) for i, w in enumerate(words)}  # 0 = mask
    return word2idx
